...
###---------------load pretrained weights--------------
    ...
    model = model.cuda()
    parameter_values = torch.load('./weights/vgg16.pth').values()
    model_dict = model.state_dict()

    for name, param in zip(model_dict.keys()[0:26], parameter_values[0:26]):
        model_dict[name] = param
    model.load_state_dict(model_dict)

     # Optimizer
    optimizer = torch.optim.SGD([{'params':model.context_aware_module.parameters()},{'params':model.features.parameters(), 'lr':learning_rate/10}], lr=learning_rate, momentum=0.9, weight_decay=1e-4, nesterov=True)
...
# ---------------------resume training------------
        ...
        if config['resume_train'] and epoch == 1:
            ...
            resume_dict = torch.load('./weights/model_' + str(load_epoch) + '.pkl')        
           ...
           #Optimizer
            optimizer = torch.optim.SGD([{'params':model.context_aware_module.parameters()},{'params':model.features.parameters(), 'lr':learning_rate / 10.0}], lr=learning_rate, momentum=0.9, weight_decay=1e-4, nesterov=True)
